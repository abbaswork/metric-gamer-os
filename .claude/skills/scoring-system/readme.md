...
| User Personas | company/persona/persona.md | 3 |
| Scoring System Overview | scoring-system/overview.md | 4 |
| Company Overview | company/overview.md | 3 |
...
| `sub_genre` | string | No | Either `multiplayer` or `free` — the two supported sub-genre types. If provided, check `scoring-system/genres/{genre}/niches/{sub_genre}/` for existing rubrics before generating new ones. Generate new rubrics only for metrics not already defined there, and write them to that same folder. |
